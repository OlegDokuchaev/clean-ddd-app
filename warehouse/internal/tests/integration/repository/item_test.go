...
func (s *ItemRepositoryTestSuite) TestGetAllByIDs() {
	tests := []struct {
		name          string
		setup         func(productRepo productDomain.Repository, itemRepo itemDomain.Repository) []uuid.UUID
		expectedError error
	...
	for _, tc := range tests {
		s.Run(tc.name, func() {
			...
			items, err := itemRepo.GetAllByIDs(s.ctx, itemIDs...)
			...
			if tc.expectedError != nil {
				...
				require.NoError(s.T(), err)
				require.Equal(s.T(), len(itemIDs), len(items))
				for _, item := range items {
					require.Contains(s.T(), itemIDs, item.ID)
				}
			}
		})
	}
